...
class BukuController extends Controller
{
    ...
    public function index() {
        ...
        $rating1 = Buku::sum('rating_1');
        $rating2 = Buku::sum('rating_2')*2;
        $rating3 = Buku::sum('rating_3')*3;
        $rating4 = Buku::sum('rating_4')*4;
        $rating5 = Buku::sum('rating_5')*5;

        $jumlah_rating = Buku::sum('rating_1') + Buku::sum('rating_2') + Buku::sum('rating_3') + Buku::sum('rating_4') + Buku::sum('rating_5');

        if($jumlah_rating > 0){
            $avg_rating = ($rating1 + $rating2 + $rating3 + $rating4 + $rating5) / $jumlah_rating;
        }
        else{
            $avg_rating = 0;
        }
        ...
        return view('buku.index', compact('data_buku','no', 'jumlah_buku', 'jumlah_harga', 'avg_rating'));
    }
